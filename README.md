...
| API·AI 서버·환자 앱·의료진 웹 코드 | 제공 |
| 배포 패키지·시연 영상 | **공개 예정** |

**국산 LLM 다중 에이전트 기반 정신건강 사전문진·종단 분석·의료진 인계 시스템**

2026 인공지능 챔피언 국내 AI 트랙 본선. 2026년 7월 시제품 평가 완료.

## 시스템 구조

![NeuroSync 입력·처리·인계 구조](figures/overview.png)

*대화·음성·문서·PHR 입력에서 의료진 인계까지의 처리 흐름.*
...
| F5 | 근거 검증과 보고서 조립 | 진단을 포함하지 않는 의료진 인계 보고서 |

![NeuroSync 전문 에이전트 구성](figures/agents.png)

*규칙 기반 오케스트레이터 1종과 전문 에이전트 10종.*
...
**평가 대상은 합성 환자 페르소나 8종이다.** 총 384세션은 모델 비교 216세션, 단일 백본·오케스트레이션 비교 144세션, 호출 구조 통제군 24세션으로 구성된다. 임상 유효성 평가는 후속 단계다.

### 단일 백본과 오케스트레이션 비교

단일 백본과 NeuroSync 각각 72세션.
...
*환자 앱, 플랫폼 백엔드, AI 서버, 데이터베이스, 의료진 대시보드.*

</details>

## 구현 및 로컬 개발

API, AI 서버, 환자 앱, 의료진 웹과 서비스 간 공유 계약을 모노레포로 관리한다.
...
개발 환경: Node.js 20.11 이상, pnpm 9.15.0, uv, Docker Compose, Make. 서비스별 환경변수와 API 키를 설정한 뒤 저장소 루트에서 [Makefile](Makefile)의 개발 명령을 실행한다.
...
`make bootstrap`은 JS 의존성과 Python 가상환경을 구성한다. `make dev`는 PostgreSQL·Redis와 개발 서비스를 실행한다. 테스트·타입 검사·린트:
