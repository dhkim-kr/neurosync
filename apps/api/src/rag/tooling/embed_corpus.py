...
def _run(cur, conn, table: str, id_col: str, text_sql: str, label: str) -> None:
    ...
    for i in range(0, len(rows), BATCH):
        ...
        for (rid, _), v in zip(chunk, vecs):
            cur.execute(
                f"UPDATE rag.{table} SET embedding=%s::vector, embedding_model=%s WHERE {id_col}=%s",
...
def main() -> None:
    with connect() as conn, conn.cursor() as cur:
        ...
        _run(
            cur, conn, "symptom", "symptom_id",
            "coalesce(name_ko,name) || ': ' || coalesce(synonyms::text,'')", "symptom",
        )
